...
ORG_PASS=Smooch!991
curl -s -u "$ORG_USER:$ORG_PASS" https://api.bitbucket.org/2.0/repositories/?role=member | jq -r '.values[] | {slug:.slug, links:.links.clone[] } | select(.links.name=="https") | "git clone \(.links.href) \(.slug)-cloud"' >> clone-repos.sh
